...
```shell
go get github.com/ondewo/ondewo-survey-client-go/v2
```
...
> `v2.y.z` tag on a module declared without `/v2` is invisible to `go get`. This release is
> `2.0.0`, so the module declares — and every generated file imports — `github.com/ondewo/ondewo-survey-client-go/v2`.
> The `Makefile` derives the suffix from `ONDEWO_SURVEY_VERSION`; run `make TEST` to print the
