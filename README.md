<div align="center">
  ...
  <h1 align="center">
    ONDEWO SURVEY Client Go
  </h1>
</div>

## Overview

`ondewo-survey-client-go` is the Go gRPC client of the ONDEWO SURVEY (Survey) API. It is a
compiled version of the [ONDEWO SURVEY API](https://github.com/ondewo/ondewo-survey-api),
...
```shell
go get github.com/ondewo/ondewo-survey-client-go/v2@latest   ## or @v2.0.0 to pin an exact release
```

Then import the package of the service you need:

```go
import surveypb "github.com/ondewo/ondewo-survey-client-go/v2/api/ondewo/survey"
```

> **The `/v2` is part of the name, not a version selector.** From major version 2 on, a Go module
...
> shorter spelling of the same module — `go get github.com/ondewo/ondewo-survey-client-go` names a
> *different*, unpublished module and fails with `invalid version: module contains a go.mod file, so
> major version must be compatible`. The suffix moves with the major version of the ONDEWO SURVEY API,
> so a `3.x` release will be imported as `/v3`, and a program can depend on both at once.

Releases are tagged twice on the same commit: with the ONDEWO release number (`2.0.0`), which is
what the [GitHub releases page](https://github.com/ondewo/ondewo-survey-client-go/releases) lists and
what the rest of the ONDEWO client fleet uses, and with the `v`-prefixed spelling (`v2.0.0`), which
...
```shell
git clone https://github.com/ondewo/ondewo-survey-client-go.git   ## Clone the repository
cd ondewo-survey-client-go                                        ## Change into the repo directory
...
import (
    ...
    surveypb "github.com/ondewo/ondewo-survey-client-go/v2/api/ondewo/survey"
    "github.com/ondewo/ondewo-survey-client-go/v2/auth"
)
...
func main() {
    ...
    conn, err := grpc.NewClient(
        "grpc-survey.ondewo.com:443",
        grpc.WithTransportCredentials(credentials.NewTLS(&tls.Config{})),
        auth.WithBearerToken(os.Getenv("ONDEWO_SURVEY_ACCESS_TOKEN")),
    )
    if err != nil {
    ...
    // api/ondewo/survey/ for the two services this product exposes: Surveys and FHIR.
    client := surveypb.NewSurveysClient(conn)

    response, err := client.ListSurveys(ctx, &surveypb.ListSurveysRequest{})
    if err != nil {
        log.Fatalf("rpc failed: %v", err)
    }
    log.Printf("surveys: %v", response.GetSurveys())
}
```
...
├── ondewo-survey-api                             <----- submodule @ https://github.com/ondewo/ondewo-survey-api
...
make generate_ondewo_protos                 ## generate api/ from ondewo-survey-api/ondewo
...
* The image copies the mounted input volume into a temporary directory inside the container and
  compiles there, so the protos of `ondewo-survey-api` are never modified.
...
* `Survey` — scalars, a nested message, a repeated message carrying a `oneof` and a packed
  repeated enum — survives `proto.Marshal` → `proto.Unmarshal` unchanged, and a truncated
  payload is rejected;
* **no** proto3 `optional` assertion, deliberately. The sibling go clients pin that an
  explicitly set zero value survives the wire — the distinction the angular target of the same
  compiler once lost. The ONDEWO SURVEY API declares no `optional` field at all (the `oneof`s
  in `survey.proto` are a different field kind), so there is nothing here to assert it against
  and the case is absent rather than faked; `tests/api_surface_test.go` says so at the point
  where it would go;
* the enum zero values are pinned with their generated name/value maps: `SubFlow(0)` is
  `SUBFLOW_UNSPECIFIED`, and `Survey.AgentStatus(0)` is the real state `TO_BE_INITIALIZED` —
  an enum whose zero member is a genuine choice, so a member inserted at 0 upstream would
  silently re-label every stored zero;
...
* every generated `New<Service>Client` binds to a connection, and every generated **unary** stub is
  actually called over the wire and has to come back as `codes.Unimplemented` — 13 of them
  across the 2 services of this product, which proves each one marshals its request and builds a
...
* all 2 compiled `.proto` files are registered in the global descriptor registry as proto3.
...
(**30.2%** of generated statements at the time of writing) for the record. `make test_coverage`
...
— the ONDEWO release tag (`2.0.0`) and the `v`-prefixed tag Go tooling requires (`v2.0.0`) — creates
...
* it must be spelled `v<semver>` — `2.0.0` alone is not a Go module version;
...
```shell
make check_go_module_path   ## go.mod and every self-import carry the /vN that ONDEWO_SURVEY_VERSION implies
