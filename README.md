...
```shell
go get github.com/ondewo/ondewo-survey-client-go
```

Then import the package of the service you need:

```go
import surveypb "github.com/ondewo/ondewo-survey-client-go/api/ondewo/survey"
```
...
> import path of release `7.1.2` is `github.com/ondewo/ondewo-survey-client-go/v7/api/ondewo/survey`. The
> `Makefile` derives the suffix from `ONDEWO_SURVEY_VERSION`; run `make TEST` to print the
...
import (
    ...
    surveypb "github.com/ondewo/ondewo-survey-client-go/api/ondewo/survey"
)
...
func main() {
    ...
    bearerToken := os.Getenv("ONDEWO_SURVEY_ACCESS_TOKEN")
    ...
    // api/ondewo/survey/ for the ones this product exposes.
    client := surveypb.NewExampleServiceClient(conn)

    response, err := client.ExampleMethod(ctx, &surveypb.ExampleRequest{})
    if err != nil {
