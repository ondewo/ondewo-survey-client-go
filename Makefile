...
# =====================================================================================
# ondewo-survey-client-go - Makefile
#
# The ONDEWO SURVEY (Survey) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-survey-api submodule by the
...
#   make test                     # go test ./...
#
# Versioning: ONDEWO_SURVEY_VERSION (below) is the single source of truth and MUST
# match the ONDEWO SURVEY API in major and minor version.
#
...
# carry the major as a `/vN` suffix (https://go.dev/ref/mod#major-version-suffixes): a v2.1.2
# tag on a module declared without `/v2` is invisible to `go get`. Derived from the version
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-survey-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_SURVEY_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_SURVEY_VERSION}
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO SURVEY Go Client ${ONDEWO_SURVEY_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-survey-client-go"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-survey-client-go repo
	pre-commit install
...
print_version: ## Print ONDEWO_SURVEY_VERSION and nothing else
	@echo "${ONDEWO_SURVEY_VERSION}"
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-survey-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-survey-client-go@v2.0.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_SURVEY_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_SURVEY_VERSION=${ONDEWO_SURVEY_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_SURVEY_VERSION=${ONDEWO_SURVEY_VERSION}"
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO SURVEY Go Client ${ONDEWO_SURVEY_VERSION}' entry"; \
